...
public interface CustomerBidRepository extends JpaRepository<CustomerBid, Long>{
	...
	@Modifying
	@Query("UPDATE CustomerBid b SET b.recipient=?2, b.status=1 where b.id=?1 AND b.status=0 AND b.recipient=null")
	int updateCustomerBidRecipientById(Long id, Customer recipient);
}
